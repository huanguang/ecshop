...
if($_REQUEST['act'] == 'dizhi'){
    $id = $_POST['id'];
    $sql = "select * from ". $ecs->table('user_address') . "where  address_id = ".$id;
    $dizhis = $db->getRow($sql);
    //var_dump($dizhis);die;
    $_SESSION['flow_consignee'] = $dizhis;die;
}
...
elseif ($_REQUEST['act'] == 'zhngjia')
{
    ...
    $moren = $_POST['moren'];
    //插入收获信息
    $sql = "insert into ".$ecs->table('user_address')."(consignee,country,province,city,district,address,zipcode,mobile,user_id) values('$consignee','$country','$province','$city','$district','$address','$zipcode','$mobile','$user_id')";
    $db->query($sql);
    //判断是否设置默认地址，如果设置的话就设置默认地址
    $id = mysql_insert_id();
    $sql = "update ".$ecs->table('users')." set address_id = '$id' where user_id = '$user_id'";
    $db->query($sql);
    show_message('地址添加成功');
