...
if(!$user_id || intval($user_id) == 0){
    ...
    if($res){
        //清空购物车中的原有数据
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_info');
        //. " WHERE ".
                //" user_id = '$user_id' and group_id = '" . $group . "'";
        $GLOBALS['db']->query($sql);
        //插入新的数据
        if($_REQUEST['way'] == 2){
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('cart_info') . " SELECT rec_id,user_id,session_id,goods_id,goods_sn,product_id,group_id,goods_name,market_price,goods_price,goods_number,goods_attr,is_real,extension_code,parent_id,rec_type,is_gift,is_shipping,can_handsel,goods_attr_id FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE ".
                "  group_id = '" . $group . "'";
            
        }else{
            $sql = "select * from " . $GLOBALS['ecs']->table('cart') . " where user_id = '$user_id' and group_id = '$group'";
            ...
            if(empty($goods_list)){
                ...
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " set goods_number = goods_number+$number WHERE ".
                "  group_id = '" . $group . "'";
            }
