...
function addToCartShowDiv2(goodsId, script_name,goods_recommend,parentId)
{
  ...
  Ajax.call('flow.php?step=add_to_cart_showDiv', 'goods=' + $.toJSON(goods), addToCartShowDivResponse2, 'POST', 'JSON');
 ...
 * 处理添加商品到购物车并且停留当前页显示出DIV反馈信息
 */
function addToCartShowDivResponse2(result)
{
  if (result.error > 0)
  {
    // 如果需要缺货登记，跳转
  ...
  if(result.script_name == 1)
  {
    $("#addtocartdialog_retui2_"+result.goods_id+goods_recommend).html(result.show_info);
    
    $("#addtocartdialog_retui2_"+result.goods_id+goods_recommend).show();
  }
  else
  {
    $("#addtocartdialog .center_pop_txt").html(result.show_info);
