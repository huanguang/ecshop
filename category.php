...
//查询所对应的品牌信息
$sql = " select brand_name from " .$ecs->table("brand"). " where brand_id = '$brand'";
$brand_name = $db->getOne($sql);
$smarty->assign('brand_name',       $brand_name);
$smarty->assign('cat_id',       $cat_id);

$price_min = isset($_REQUEST['price_min']) && intval($_REQUEST['price_min']) > 0 ? intval($_REQUEST['price_min']) : 0;
$cuxiao = isset($_REQUEST['cuxiao']) && intval($_REQUEST['cuxiao']) > 0 ? intval($_REQUEST['cuxiao']) : 0;
$baoyou = isset($_REQUEST['baoyou']) && intval($_REQUEST['baoyou']) > 0 ? intval($_REQUEST['baoyou']) : 0;
$pin = isset($_REQUEST['pin']) ? $_REQUEST['pin'] : '';
$shux = isset($_REQUEST['shux']) ? $_REQUEST['shux'] : '';
$pin = explode(',',$pin);
$shux = explode(',',$shux);
...
foreach ($shux as $key => $value) {
    if(empty($shux[$key])){
        unset($shux[$key]);
    }
}
 $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 39 AND enabled = 1 ';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_fen = $db->getRow($sql);
    $smarty->assign('ad_fen',       $ad_fen);
$smarty->assign('cuxiao',       $cuxiao);
$smarty->assign('baoyou',       $baoyou);
...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
    ...
    //明星彩妆广告轮播图5张
    $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 29 AND enabled = 1 LIMIT 5';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_cz29 = $db->getAll($sql);
    $smarty->assign('ad_cz29',$ad_cz29);

    //明星彩妆美妆教师
    $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 30 AND enabled = 1';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_czjs = $db->getRow($sql);
    $smarty->assign('ad_czjs',$ad_czjs);

    //明星彩妆保健常识
    $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 31 AND enabled = 1';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_czcs = $db->getRow($sql);
    $smarty->assign('ad_czcs',$ad_czcs);

    //明星彩妆导师推荐
    $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 32 AND enabled = 1';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_cztt = $db->getRow($sql);
    $smarty->assign('ad_cztt',$ad_cztt);
    ...
    //var_dump(get_category_recommend_goods('best', $children, $brand, $price_min, $price_max, $ext));die;
    ...
    $count = get_cagtegory_goods_count($children, $brand, $price_min, $price_max, $ext,$cuxiao,$baoyou,$pin,$shux);
    $max_page = ($count> 0) ? ceil($count / $size) : 1;
    if ($page > $max_page)
    {
        $page = $max_page;
    }
    $goodslist = category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order,$cuxiao,$baoyou,$pin,$shux);
    if($display == 'grid')
    {
    ...
    foreach ($goodslist as $key => $value) {
        if($value){
            $sql = " select count(*) from ".$ecs->table('order_goods')." as a LEFT JOIN ".$ecs->table('order_info')." as b on a.order_id = b.order_id where b.shipping_status = 2 and a.goods_id = ".$value['goods_id'];
        $dd = $db->getOne($sql);
        $goodslist[$key]['yishou'] = $dd[0];
        //统计所有评价
        $sql = "select count(*) from ".$ecs->table('comment')." where id_value = ".$value['goods_id'];
        $pin_z = $db->getOne($sql);
        //var_dump($pin_z);
        //统计所有的好好评
        $goodslist[$key]['properties'] = get_goods_properties($value['goods_id']);  // 获得商品的规格和属性
        $sql = "select count(*) from ".$ecs->table('comment')." where comment_rank = 5 and id_value = ".$value['goods_id'];
        $pin_h = $db->getOne($sql);
        if($pin_z[0] == 0){
           $goodslist[$key]['haopin'] = 100; 
        }else{
            $goodslist[$key]['haopin'] = ($pin_h[0]/$pin_z[0])*100;
        }
        //获取商品属性
...
if($cat_id == '167'){
    $smarty->display('category_xins.dwt', $cache_id);
}else{
 ...
 * @return  array
 */
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order,$cuxiao,$baoyou,$pin=array(),$shux=array())
{
    ...
    if(!empty($pin)){
        foreach ($pin as $key => $value) {
            $sql = "select brand_id from "  . $GLOBALS['ecs']->table('brand') . " where brand_name = '$value'";
            $id_brand  = $GLOBALS['db']->getOne($sql);
            $data[] = $id_brand;
        }
    }
    ...
    if ($baoyou > 0)
    {
        $where .= " AND g.is_shipping = $baoyou ";
    }
    ...
    if($cuxiao > 0){
        $where .= "and g.promote_start_date > 0 and promote_end_date > 0";
    }
    if(!empty($pin)){
        $shuj = '';
        foreach ($data as $key => $value) {
            $shuj .= "g.brand_id='$value' || "; 
        }

        $sql = 'SELECT g.goods_id, g.goods_name,g.goods_name_e,g.goods_qitajie, g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE $where and ($shuj g.brand_id = '0') $ext ORDER BY $sort $order";

    }elseif(!empty($shux)){
        $shi_info = ' ';
        foreach ($shux as $key => $value) {
            $shi_info .= "c.attr_value = '$value' || "; 
        }
        //$shis = $shi;
        $asd = "left JOIN".$GLOBALS['ecs']->table('goods_attr')." as c on g.goods_id = c.goods_id";
        $sql = 'SELECT g.goods_id, g.goods_name,g.goods_name_e,g.goods_qitajie, g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .$asd.
            " WHERE $where and ($shi_info c.attr_value = 'ewqe') $ext ORDER BY $sort $order";
    }else{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['goods_name_e']             = $row['goods_name_e'];
        $arr[$row['goods_id']]['goods_qitajie']             = $row['goods_qitajie'];
        ...
        $sql =" select count(*) from " .$GLOBALS['ecs']->table('comment'). " where comment_rank = 5 and id_value = ".$row['goods_id'];
             ...
             $arr[$row['goods_id']]['count_comment'] = ($count_comment_hao/$count_comment_zong)*100;
             //添加销量
          $arr[$row['goods_id']]['count_xiaoliang']  = get_goods_count($row['goods_id']);
    }
 ...
 * @return  integer
 */
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='',$cuxiao =0,$baoyou =0)
{
