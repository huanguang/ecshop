...
<div class="wrap"> 
  ...
  <?php $_from = $this->_var['consignee_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('sn', 'consignee_0_06124500_1426737381');if (count($_from)):
    foreach ($_from AS $this->_var['sn'] => $this->_var['consignee_0_06124500_1426737381']):
?>
...
?>
        ...
        <?php endif; ?> 
		 ...
		 <?php if ($this->_var['allow_use_bonus']): ?>
        <dl>
          <dd class="dd1">现金劵号 </dd>
          <dd>  <input name="bonus_sn" type="text" class="inputBg" size="15" value="<?php echo $this->_var['order']['bonus_sn']; ?>" />
                <input name="validate_bonus" type="button" class="bnt_blue_1" value="<?php echo $this->_var['lang']['validate_bonus']; ?>" onclick="validateBonus(document.forms['theForm'].elements['bonus_sn'].value)" style="vertical-align:middle;" /></dd>  
          <i></i>
        </dl>
		<div class="dl_box" id="bonus_box" style="display:none;">
          <?php $_from = $this->_var['bonus_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'bonus');if (count($_from)):
