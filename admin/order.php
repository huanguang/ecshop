...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        $sql = "select order_sn,shipping_name,user_id from " .$GLOBALS['ecs']->table('order_info'). " where order_id = '$order_id'";
        $orderss = $GLOBALS['db']->getRow($sql);
        $sql = "select user_name from ecs_users where user_id = ".$orderss['user_id'];
        $username = $GLOBALS['db']->getOne($sql);


        $content = '您的订单('.$orderss[order_sn].')卖家已交付'.$orderss['shipping_name']. '运单号为：'.$_REQUEST['invoice_no'];
        $sql = "INSERT INTO ecs_news (j_user_id,add_time,type,content,is_kan) VALUES($orderss['user_id'],".gmtime().",2,'$content',0)";
        $db->query($sql);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn,o.is_tui, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
