...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    $order_list = zixun();
    $smarty->assign('order_list',   $order_list['msg']);
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('zixun_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    //将这个会员的回复信息改为已查看状态
    $sql = " update " .$ecs->table('message'). " set is_kan = 1 where user_id = $userid and status = 0";
    $db->query($sql);
    //查询该会员的所有在线咨询的信息
    $sql = "select a.*,b.user_name from " .$ecs->table('message'). " as a LEFT JOIN " .$ecs->table('users') . " as b on a.user_id = b.user_id where a.user_id = " .$userid . " order by a.add_time asc";
    $msgs = $db->getAll($sql);

    foreach ($msgs as $key => $value) {
        $msgs[$key]['add_time'] = local_date('Y-m-d H:i:s',$value['add_time']);
    }
    
        /* 模板赋值 */
        $smarty->assign('msg', $msgs);
        $smarty->assign('ur_here', '咨询详情');
        $smarty->assign('action_link', array('href' => 'zixun.php?act=list&' . list_link_postfix(), 'text' => '在线咨询列表'));

        /* 显示模板 */
        assign_query_info();
        $smarty->display('zixun_info.htm');
}
...
//-- 后台回复会员的信息
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'huifus')
{
    $huifu = $_REQUEST['huifu'];
    $uid = $_REQUEST['uid'];

    //执行会员信息回复操作
    $sql = "insert into " .$ecs->table('message') . " (user_id,message,add_time,status) values('$uid','$huifu','".gmtime()."','1')";
    if($db->query($sql)){
        header("Location:zixun.php?act=list"); 
    }else{
        header("Location:zixun.php?act=list"); 
    }
}
...
function zixun()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['order_sn'])
        {
            $where .= " AND message LIKE '%" . $filter['order_sn']. "%'";
        }
        ...
        /* 记录总数 */
            ...
            $sql = "select count(*) from (select a.*,b.user_name from ecs_message as a LEFT JOIN ecs_users as b on a.user_id = b.user_id order by a.id desc) t " .$where. " group by user_id ";
        ...
        /* 查询 */
        $sql = "select * from (select a.*,b.user_name from ecs_message as a LEFT JOIN ecs_users as b on a.user_id = b.user_id order by a.id desc) t " .$where. " group by user_id " .
        //echo $sql = "select a.*,u.user_name from " .$GLOBALS['ecs']->table('message'). " as a " .
                //" LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=a.user_id where a.user_id in ( selelct max(id) from " .$GLOBALS['ecs']->table('message'). " group by user_id ) " . 
    ...
    $arr = array('msg' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
