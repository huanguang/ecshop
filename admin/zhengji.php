...
{
    $ranks = array();
    $ranks = $db->getAll("SELECT * FROM " .$ecs->table('solicitation'));

    $smarty->assign('ur_here','征集列表');
    $smarty->assign('action_link',  array('text' => '添加征集', 'href'=>'user_rank.php?act=add'));
    $smarty->assign('full_page',    1);

    $smarty->assign('user_ranks',   $ranks);

    assign_query_info();
    $smarty->display('zhengji_list.htm');
}
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->display('zhengji_edit.htm');
