...
$not_login_arr =
array('login','act_login','register','check_verify','zhenshi','mmxg','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','check_phones','qpassword_name','qiandao', 'get_passwd_question', 'check_answer', 'oath' , 'oath_login', 'other_login');
...
if($action == 'check_verify'){
        //echo 123;
        if(empty($_POST) || $_POST['phone_num'] == ''){
        ...
        echo $url = "http://dx.10659com.com:83/ApiService.asmx/Send&mobile=".$phone_num."&message=".urlencode($msg);die;
        //echo $url;
        $post = is_post($url);//封装函数，post的方式去访问接口地址

        if($post == 000){//000是接口的返回值
            echo 1;
        }else{
            echo 0;
        }
    ...
    if($_POST){
        ...
        if($status != '0'){
            ...
            if($status == '待收货'){
                ...
                $where .= "order_status = 1 and shipping_status = 0 and pay_status = 2  and ";
            }elseif($status == '待付款'){
                $where .= "order_status = 0 and shipping_status = 0 and pay_status = 0  and ";
            }
