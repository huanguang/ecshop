...
{
    $ranks = array();
    $ranks = $db->getAll("SELECT * FROM " .$ecs->table('join_us'));

    $smarty->assign('ur_here','加盟列表');
    $smarty->assign('action_link',  array('text' => '添加加盟列表', 'href'=>'user_rank.php?act=add'));
    $smarty->assign('full_page',    1);

    $smarty->assign('user_ranks',   $ranks);

    assign_query_info();
    $smarty->display('jiameng_list.htm');
}
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $id = intval($_POST['id']);
    $sql = "delete from ".$ecs->table('join_us')." where id = '$id'";
    if($db->query($sql)){
    	sys_msg('删除成功');
    }else{
    	sys_msg('删除失败');
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $id = intval($_GET['id']);
    $sql = "select * from ".$ecs->table('join_us')." where id = '$id'";

    $zhengji = $db->getRow($sql);
    $smarty->assign('jiameng',$zhengji);
    $smarty->display('jiameng_edit.htm');
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $id = intval($_POST['id']);
    $sql = "update ".$ecs->table('join_us')." set name='$_POST[name]',email='$_POST[email]',dizhi='$_POST[dizhi]',iphone='$_POST[iphone]' where id = '$id'";
    if($db->query($sql)){
    	sys_msg('修改成功');
    }else{
    	sys_msg('修改失败');
