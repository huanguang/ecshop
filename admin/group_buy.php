...
elseif ($_REQUEST['act'] =='insert_update')
{
    ...
    elseif (isset($_POST['succeed']))
    {
        ...
        // $sql = "select * from ecs_goods_activity where act_id = ".intval($_POST['act_id']);
        // $tuan = $db->getRow($sql);
        // if($tuan['start_time'] < gmtime() && $tuan['end_time'] > gmtime()){
        //     //推送消息
        //     require_once(dirname(__FILE__) . "./../jpushapi/src/JPush/JPush.php");
        //     $content = '现在有新的团购商品（'.$tuan[goods_name].'）正在进行';
        //     $br = '<br/>';
    ...
    else
    {
        ...
        $group_buy['act_desc2'] = $_POST['act_desc2'];
        //var_dump($group_buy);die;
        /* 清除缓存 */
        clear_cache_files();
    ...
    if($tuan['start_time'] < gmtime() && $tuan['end_time'] > gmtime()){
        ...
        else
        {
            ...
            $sql = "select * from ecs_goods_activity order by act_id desc";
    $tuan = $db->getAll($sql);
    if($tuan[0]['start_time'] < gmtime() && $tuan[0]['end_time'] > gmtime()){
        //推送消息
        
        require_once(dirname(__FILE__) . "./../jpushapi/src/JPush/JPush.php");
        $content = '现在有新的团购商品（'.$tuan[0][goods_name].'）正在进行';
        $br = '<br/>';
        $app_key = 'dd1066407b044738b6479275';
        $master_secret = 'e8cc9a76d5b7a580859bcfa7';
