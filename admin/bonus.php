...
if ($_REQUEST['act'] == 'update')
{
    ...
    $use_startdate  = local_strtotime($_POST['use_start_date']);
    $use_enddate    = local_strtotime($_POST['use_end_date']);
    if ((isset($_FILES['youhui_img']['error']) && $_FILES['youhui_img']['error'] == 0) || (!isset($_FILES['youhui_img']['error']) && isset($_FILES['youhui_img']['tmp_name'] ) &&$_FILES['youhui_img']['tmp_name'] != 'none'))
        {
            $youhui_code = basename($image->upload_image($_FILES['youhui_img'], 'youhuijuan'));
        }
    /* 对数据的处理 */
    $type_name   = !empty($_POST['type_name'])  ? trim($_POST['type_name'])    : '';
    $type_id     = !empty($_POST['type_id'])    ? intval($_POST['type_id'])    : 0;
    $min_amount  = !empty($_POST['min_amount']) ? intval($_POST['min_amount']) : 0;
    $duihuanjifen  = !empty($_POST['duihuanjifen']) ? intval($_POST['duihuanjifen']) : 0;
    ...
    $sql = "UPDATE " .$ecs->table('bonus_type'). " SET ".
           ...
           "duihuanjifen  = '$duihuanjifen', ".
           "youhui_img  = '$youhui_code', ".
