...
function order_goods($order_id)
{
	...
	foreach($goods_list as $key=> $value){
		$sql = "select goods_img from". $GLOBALS['ecs']->table('goods') ." where goods_id = ".$value['goods_id'];
		$goods_list[$key]['goods_img'] = $GLOBALS['db']->getOne($sql);
	}
    //return $GLOBALS['db']->getAll($sql);
    return $goods_list;
}
 ...
 * @return  array
 */
function order_fee2($order, $goods, $consignee,$user_id)
{
    /* 初始化订单的扩展code */
    if (!isset($order['extension_code']))
    {
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            // 查看购物车中是否全为免运费商品，若是则把运费赋为零
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart_info') . " WHERE  `user_id` = '" . $user_id. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
            $shipping_count = $GLOBALS['db']->getOne($sql);
    ...
    {
        $sql = 'SELECT SUM(eg.exchange_integral) '.
               'FROM ' . $GLOBALS['ecs']->table('cart_info') . ' AS c,' . $GLOBALS['ecs']->table('exchange_goods') . 'AS eg '.
               "WHERE c.goods_id = eg.goods_id  " .//AND c.session_id= '" . SESS_ID . "'
               "  AND c.rec_type = '" . CART_EXCHANGE_GOODS . "' " .
               '  AND c.is_gift = 0 AND c.goods_id > 0 ' .
               'GROUP BY eg.goods_id';
...
function cart_goods4($type = CART_GENERAL_GOODS,$user_id,$rec_id)
{
    $sql = "SELECT c.rec_id,c.group_id,c.parent_id, c.user_id, c.goods_id, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, " .
            ...
            " AS g ON c.goods_id = g.goods_id WHERE user_id = '" . $user_id . "' and  c.rec_id in(".$rec_id.")" .
            "AND rec_type = '$type'";
    ...
    foreach ($arr as $key => $value) {
        ...
        if($value['group_id'] && $value['parent_id'] == 0){
            ...
            "FROM " . $GLOBALS['ecs']->table('cart') .
            " AS c LEFT JOIN ".$GLOBALS['ecs']->table('goods').
            " AS g ON c.goods_id = g.goods_id WHERE user_id = '" . $user_id . "' and  c.parent_id = $value[goods_id]" .
            " AND rec_type = '$type'";
            $ff[] = $GLOBALS['db']->getAll($sql);
        }
...
function cart_goods2($type = CART_GENERAL_GOODS ,$user_id,$num = 1, $start = 0)
{
    $sql = "SELECT c.rec_id, c.user_id,c.group_id,c.parent_id, c.goods_id, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, " .
            "c.market_price, c.goods_price, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " .
            "c.goods_price * c.goods_number AS subtotal " .
            "FROM " . $GLOBALS['ecs']->table('cart') .
            " AS c LEFT JOIN ".$GLOBALS['ecs']->table('goods').
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $goods_list['package_goods_list'] = get_package_goods($row['goods_id']);
        }
    }
...
function cart_goods3($type = CART_GENERAL_GOODS ,$rec_id)
{

    if(strstr($rec_id,"m_goods_1_")){
        
        $where = " WHERE c.group_id = '$rec_id' ";
    }else{
        $where = " WHERE c.rec_id = $rec_id ";
    }
    
    
    $sql = "SELECT c.rec_id,c.group_id, c.user_id, c.goods_id, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, " .
            "c.market_price, c.goods_price, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " .
            "c.goods_price * c.goods_number AS subtotal " .
            "FROM " . $GLOBALS['ecs']->table('cart_info') .
            " AS c LEFT JOIN ".$GLOBALS['ecs']->table('goods').
            " AS g ON c.goods_id = g.goods_id $where " .
            "AND c.rec_type = '$type' ";
 ...
 * @return  boolean
 */
function addto_cart2($goods_id, $num = 1, $spec = array(), $parent = 0,$user_id)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
 ...
 * @return  boolean
 */
function addto_cart3($goods_id, $num = 1, $spec = array(), $parent = 0,$user_id)
{
 ...
 $GLOBALS['err']->clean();
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            $parent['goods_number'] = $num;
            $parent['parent_id']    = 0;
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart_info'), $parent, 'INSERT');
            $sql = "select max(rec_id) as max from " .$GLOBALS['ecs']->table('cart_info');
            $id = $GLOBALS['db']->getRow($sql);
        }
        else //购物车没有此物品，则插入
        {
            $goods_price = get_final_price($goods_id, $num, true, $spec);
    ...
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_info') . " WHERE session_id = '" . SESS_ID . "' AND is_gift <> 0";
    $GLOBALS['db']->query($sql);
 ...
 * @return  boolean
 */
function addto_cart_combo2($goods_id, $num = 1, $spec = array(), $parent = 0, $group = 0,$user_id)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql = "SELECT t.type_id, t.type_name,t.use_end_date,t.min_goods_amount, t.type_money, b.bonus_id " .
