...
function get_top10_6f($cats = '')
{
    ...
    //         $top10_time = '';
    // }
    $GLOBALS['_CFG']['top10_time'] = $top10_time;
    $top10_time = "AND o.order_sn >= '" . date('Ymd', gmtime() - 30 * 86400) . "'";
    //print_R($top10_time);die;
    ...
    $sql .= ' AND og.order_id = o.order_id AND og.goods_id = g.goods_id ' .
           ...
           'GROUP BY g.goods_id ORDER BY goods_number DESC, g.goods_id DESC LIMIT 7' ;
 ...
 * @return  array
 */
function get_recommend_goods_bak($type = '', $cats = '')
{   
    ...
    if (empty($type_goods[$type]))
    {
        ...
        //print_r($type2lib);die;
        // Array ( [best] => recommend_best [new] => recommend_new [hot] => recommend_hot ) 
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.promote_end_date, g.comments_number, g.sales_volume, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        //SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume, g.market_price, g.shop_price AS org_price, g.promote_price, IFNULL(mp.user_price, g.shop_price * '1') AS shop_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd FROM `ecshop_miqi`.`ecs_goods` AS g LEFT JOIN `ecshop_miqi`.`ecs_member_price` AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = '0' WHERE g.goods_id IN ('170','17','5','9','1','24') ORDER BY g.sort_order, g.last_update DESC
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            if($row['promote_end_date'] >0){
                //设置该商品是促销商品的标记
               $goods[$idx]['promote_end_date']   = 1; 
            }
            $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
            $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
            $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
            $goods[$idx]['goods_count'] = get_goods_count($row['goods_id']);
            ...
            //赋值到一个新的字段
             ...
             $goods[$idx]['count_comment'] = ($count_comment_hao/$count_comment_zong)*100;
             //var_dump($count_comment_hao);die;
...
function get_promote_goods($cats = '')
{
    ...
    $sql .= " LIMIT $num ";
    //SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume,g.market_price, g.shop_price AS org_price, g.promote_price, IFNULL(mp.user_price, g.shop_price * '1') AS shop_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name, g.is_best, g.is_new, g.is_hot, g.is_promote, RAND() AS rnd FROM `ecshop_miqi`.`ecs_goods` AS g LEFT JOIN `ecshop_miqi`.`ecs_brand` AS b ON b.brand_id = g.brand_id LEFT JOIN `ecshop_miqi`.`ecs_member_price` AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = '0' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_promote = 1 AND promote_start_date <= '1446603940' AND promote_end_date >= '1446603940' ORDER BY g.sort_order, g.last_update DESC LIMIT 1 
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.goods_id,g.goods_brief,g.goods_liangdian,g.goods_jiany,g.keywords,g.goods_desc, g.market_price, g.comments_number ,g.sales_volume,g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods[$idx]['goods_liangdian']   = $row['goods_liangdian'];
        $goods[$idx]['goods_jiany']   = $row['goods_jiany'];
        $goods[$idx]['keywords']   = $row['keywords'];
        ...
        //添加好评率字段
             ...
             $goods[$idx]['count_comment'] = ($count_comment_hao/$count_comment_zong)*100;
         //添加销量
          $goods[$idx]['count_xiaoliang']  = get_goods_count($row['goods_id']);
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_logo,b.brand_desc, g.comments_number, g.sales_volume,b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    //SELECT g.goods_id, g.goods_name, g.market_price, g.comments_number ,g.sales_volume,g.shop_price AS org_price, IFNULL(mp.user_price, g.shop_price * '1') AS shop_price, g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img FROM `ecshop_miqi`.`ecs_goods` AS g LEFT JOIN `ecshop_miqi`.`ecs_member_price` AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = '0' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.cat_id IN ('6','8','31','28','32','29','30','9','37','34','38','35','39','36','33','40','11','44','41','45','42','43','7','21','25','22','19','26','23','20','27','24') OR g.goods_id IN ('') )
 ...
 * @return  array
 */
function get_goods_fittings_bak($goods_list = array())
{
    $temp_index = 0;
    $arr        = array();

    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.group_id, gg.goods_id, gg.goods_price, g.comments_number,g.sales_volume,g.goods_name, g.goods_thumb, g.goods_img,g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
            ...
            "ORDER BY gg.parent_id, gg.goods_id LIMIT 2";
...
* @return array 
*/ 
function index_get_class_articles_er($cat_aid, $cat_num,$num='',$start='') 
{
$sql = "SELECT article_id, title,open_type,cat_id,file_url,content,description FROM " .$GLOBALS['ecs']->table('article'). " WHERE cat_id = ".$cat_aid." and is_open = 1 ";
//$res = $GLOBALS['db']->getAll($sql);
...
while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $arr[$row['article_id']]['id'] = $row['article_id']; 
    $arr[$row['article_id']]['title'] = $row['title']; 
    $arr[$row['article_id']]['description'] = $row['description']; 
    $arr[$row['article_id']]['content'] = $row['content'];
    $arr[$row['article_id']]['file_url'] = $row['file_url'];
    $arr[$row['article_id']]['short_title'] = $GLOBALS['_CFG']['article_title_length'] > 0 ? 
    sub_str($row['title'], $GLOBALS['_CFG']['article_title_length']) : $row['title']; 
    $arr[$row['article_id']]['cat_name'] = $row['cat_name']; 
    ...
    $arr[$row['article_id']]['cat_url'] = build_uri('article_cat', array('acid' => $row['cat_id'])); 
    }
    //var_dump($arr);die;
return $arr; 
}
