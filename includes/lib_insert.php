...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<li>
                        <div class="pic"><a href="'.$goods['url'].'"><img src="'.$goods['goods_thumb'].'" width="185" height="160"></a></div>
                        <div class="wz">
                          <div class="jg">
                                <span>'.$goods['shop_price'].'</span>
                                <s></s>
                            </div>
                            <div class="tit"><a href="'.$goods['url'].'">'.$goods['goods_name'].'</a></div>
                            <div class="an">
                                <a class="gwc" href="'.$goods['url'].'"><img src="images/car.png">加入购物车</a>
                                <a class="ljgm" href="'.$goods['url'].'">立即购买</a>
                                <div class="clear"></div>
                            </div>
                        </div>
...
function insert_bought_notes($arr)
{
    ...
    /* 商品购买记录 */
    $sql = 'SELECT u.user_name, og.goods_number,og.goods_name,og.goods_price,oi.pay_time, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           ...
           'WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = ' . $arr['id'] . ' and pay_status = 2 ORDER BY oi.add_time DESC LIMIT 5';
