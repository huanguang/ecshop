 ...
 */
function get_user_account($user_id, $num = 10, $start = 0)
{
    ...
    {
        $arr[] = array('change_time'       => local_date('Y-m-d H:i:s',$value['change_time']),
 ...
 * @return  array       $order_list     订单列表
 */
function get_zhengji($num = 5, $start = 0,$where = '')
{
    $arr    = array();
    $sql = " select * from ".$GLOBALS['ecs']->table('solicitation')."WHERE is_show = 0 $where";

    $res = $GLOBALS['db']->SelectLimit($sql, $num,$start);
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $arr[] = array('school_name'       => $row['school_name'],
                       'sontact_person'       => $row['sontact_person'],
                       'contact_information'       => $row['contact_information'],
                       'recommended_reason'       => $row['recommended_reason'],
                       'parent_name'       => $row['parent_name'],
                       'tuijian_lianxi'       => $row['tuijian_lianxi'],
                       'name'       => $row['name'],
                       'content'       => $row['content'],
                       'praise'       => $row['praise'],
                       'id'       => $row['id'],
                       'add_time'     => local_date('Y-m-d:H:i:s', $row['add_time']),
                       );
    }
    return $arr;
}
...
function get_user_orders($user_id, $num = 10, $start = 0,$where = '')
{
    ...
    $sql = "SELECT o.pay_id, o.order_amount,o.order_id,o.is_comment,o.invoice_no,o.shipping_name,o.extension_code,o.order_status,o. order_sn,o.pay_name, o.shipping_status, o.pay_status, o.add_time, " .
           ...
           " FROM " .$GLOBALS['ecs']->table('order_info') .
           " as o  WHERE $where o.user_id = '$user_id'  $sql_add ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //$exists =  $GLOBALS['db'] -> getOne("select count(*) from ".$GLOBALS['ecs']->table('change')." where order_sn = '".$row['order_sn']."'");
         //判断支付状态加入支付按钮

         if(strpos($row['order_status'],'未付款'))
                       {
                          $pay_online_xianshi=1;
                       }
                       else
                       {
                          $pay_online_xianshi=0;
                       }

        // $row['shipping_status'] = ($row['shipping_status'] == SS_SHIPPED_ING) ? SS_PREPARING : $row['shipping_status'];
        // $row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']] . ',' . $GLOBALS['_LANG']['ps'][$row['pay_status']] . ',' . $GLOBALS['_LANG']['ss'][$row['shipping_status']];
        //查询订单下的所有商品
        $sql = "select a.*,b.* from " .$GLOBALS['ecs']->table('order_goods'). " as a left join ".$GLOBALS['ecs']->table('goods')." as b on a.goods_id = b.goods_id where a.order_id = ".$row['order_id'];
        $goods = $GLOBALS['db']->getAll($sql);
        foreach ($goods as $key => $value) {
            $goods[$key]['goods_len'] = count($goods);
        }
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'total_fee'      => price_format($row['total_fee'], false),
                        ...
                        'pay_online_xianshi'   => $pay_online_xianshi,
