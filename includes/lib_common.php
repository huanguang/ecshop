...
function get_goods_count($goods_id)
{
  ...
  /* 查询该商品销量 */
    $sql = 'SELECT IFNULL(SUM(g.goods_number), 0) as count ' .
        'FROM ecs_order_info AS o, ecs_order_goods AS g ' .
...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        $arr['top_number']           = intval($arr['top_number'])      > 0 ? intval($arr['top_number'])      : 9;
 ...
 * @return  array
 */
function get_promotion_info_bak($goods_id = '')
{
    $snatch = array();
    $group = array();
    $auction = array();
...
{
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number,g.goods_img, p.goods_attr, p.product_number, p.product_id
            FROM " . $GLOBALS['ecs']->table('package_goods') . " AS pg
