...
class syl
{
    ...
    function get_code($order, $payment,$cat_id)
    {
...
//	    //后台返回地址(您服务器上可访问的URL)，最长80位，当用户完成支付后，我方服务器会POST订单结果信息到该页面，必填
       ...
       // $backEndUrl            = $GLOBALS['ecs']->url().'respond.php';
        //echo basename(__FILE__, '.php');die;
        //include(ROOT_PATH. 'includes/lib_payment.php');
        //$url =$GLOBALS['ecs']->url() . 'respond.php?code=' . $code;
        //
        $code = basename(__FILE__, '.php');
        $frontEndUrl           = $GLOBALS['ecs']->url() . 'respond.php?code=' . $code;
        $backEndUrl            = $GLOBALS['ecs']->url() . 'respond.php?code=' . $code;
        //var_dump(basename(__FILE__, '.php'));die;
        ...
        $html = $this->create_html($merid , $ordid , $transamt , $curyid ,$transdate , $transtype , $order['log_id'],REQ_URL_PAY,$frontEndUrl,$cat_id);
        
        return $html;

    }

    function create_html($merid , $ordid , $transamt , $curyid ,$transdate , $transtype , $order_log_id,$act_url,$frontEndUrl,$cat_id = '')
    {
        $html = <<<eot
<form action="http://payment.ChinaPay.com/pay/TransGet" method="post" target="_blank">
...
<input type="hidden" name="GateId" value="{$cat_id}"/>
<input type="hidden" name="Priv1" value="{$order_log_id}" />
<input type="hidden" name="ChkValue" value="{$this->chkvalue}" />
<input type="image" src="themes/miqinew/images/icon_order1_03.png" style="width:80px;">
</form>
eot;
