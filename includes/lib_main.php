...
function get_shop_help_footer()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'ON a.cat_id = c.cat_id WHERE c.cat_type = 5 AND a.is_open = 1 AND a.cat_id IN(13, 14, 15, 16, 17, 18) ' .
 ...
 * @return  array
 */
function get_shop_help_left()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            'FROM ' .$GLOBALS['ecs']->table('article'). ' AS a ' .
            'LEFT JOIN ' .$GLOBALS['ecs']->table('article_cat'). ' AS c ' .
            'ON a.cat_id = c.cat_id WHERE  a.cat_id IN(24, 25, 26, 27) ' .
...
function assign_comment($id, $type, $page = 1)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //获取评论的图片
        $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment_img') .
            " WHERE comment_id = ".$row['comment_id'];
        $ress = $GLOBALS['db']->getAll($sql);
        $arr[$row['comment_id']]['img'] = $ress;
        //调用用户头像
        $sql = 'SELECT head_portrait FROM ' . $GLOBALS['ecs']->table('users') .
            " WHERE user_id = ".$row['user_id'];
        $head_portrait = $GLOBALS['db']->getOne($sql);
        $arr[$row['comment_id']]['head_portrait'] = $head_portrait;
    }
    /* 取得已有回复的评论 */
    if ($ids)
    {
 ...
 * @return  array
 */
function assign_comment2($id, $type, $page = 1,$rank = '')
{
    /* 取得评论列表 */
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value = '$id' $rank AND comment_type = '$type' AND status = 1 AND parent_id = 0");
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
        //获取评论的图片
        // $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment_img') .
        //     " WHERE comment_id = ".$row['comment_id'];
        // $ress = $GLOBALS['db']->getAll($sql);
        ...
        $sql = 'SELECT head_portrait FROM ' . $GLOBALS['ecs']->table('users') .
            " WHERE user_id = ".$row['user_id'];
        $head_portrait = $GLOBALS['db']->getOne($sql);
        $arr['head_portrait'] = $head_portrait;
    ...
    if ($ids)
    {
        ...
        {
            //$arr[$row['parent_id']]['re_content']  = nl2br(str_replace('\n', '<br />', htmlspecialchars($row['content'])));
            $arr['re_content']  = nl2br(str_replace('\n', '<br />', htmlspecialchars($row['content'])));
            $arr['re_add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
            $arr['re_email']    = $row['email'];
            $arr['re_username'] = $row['user_name'];
        }
    }
    /* 分页样式 */
    //$pager['styleid'] = isset($GLOBALS['_CFG']['page_style'])? intval($GLOBALS['_CFG']['page_style']) : 0;
    ...
    $cmt = array('comments' => $arrs, 'pager' => $pager);
