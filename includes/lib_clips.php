 ...
 */
function get_booking_list($user_id, $num, $start,$where = '')
{
    $booking = array();
    $sql = "SELECT bg.rec_id, bg.goods_id, bg.goods_number,bg.order_sn, bg.booking_time, bg.dispose_note, g.goods_name ".
           "FROM " .$GLOBALS['ecs']->table('booking_goods')." AS bg , " .$GLOBALS['ecs']->table('goods')." AS g". " WHERE " .$where. " bg.goods_id = g.goods_id AND bg.user_id = '$user_id' ORDER BY bg.booking_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $order['goods_amount'] = '';
        if($row['order_sn']){
        $sql = "select * from "  .$GLOBALS['ecs']->table('order_info')." where order_sn = ".$row['order_sn'];
...
function add_booking($booking)
{
    $sql = "INSERT INTO " .$GLOBALS['ecs']->table('booking_goods').
            "(user_id,email,link_man,tel,goods_id,goods_desc,goods_number,booking_time) VALUES ('$_SESSION[user_id]', '$booking[email]', '$booking[linkman]', ".
                "'$booking[tel]', '$booking[goods_id]', '$booking[desc]', ".
                "'$booking[goods_amount]', '".gmtime()."')";
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money,head_portrait, credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    ...
    $info['head_portrait'] = $row['head_portrait'];
...
{
    $sql = "SELECT c.*, g.goods_name AS cmt_name,r.id_value as ss,r.user_id as cc, r.content AS reply_content, r.add_time AS reply_time ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //查询该订单是第一次评论还是首次评论
        if(!empty($row['ss']) && !empty($row['cc'])){
        $sql ="select a.*,b.* from " .$GLOBALS['ecs']->table('order_info'). " as a LEFT JOIN " .$GLOBALS['ecs']->table('order_goods'). " as b on a.order_id = b.order_id  where b.goods_id = ".$row['ss']." and a.user_id = ".$row['cc'];
        $goods_comment = $GLOBALS['db']->getRow($sql);
        $row['is_comment'] = $goods_comment['is_comment'];
        }
        $comments[] = $row;
    }
