...
$not_login_arr =
array('login','act_login','register','check_verify','zhenshi','mmxg','act_register','yanzhneg','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','check_phones','qpassword_name','qiandao', 'get_passwd_question', 'check_answer', 'oath' ,'tuihuan', 'oath_login', 'other_login');

/* 显示页面的action列表 */
$ui_arr = array('register','mmxg', 'login', 'profile','jifen', 'order_list','zj', 'order_detail', 'address_list', 'collection_list',
...
if($action == 'check_verify'){
   ...
   if(empty($phone) ){
        ajaxReturn(array('ret_code' => 0, 'msg' => '请输入正确的手机号码', 'data' => array()),'JSON');
    }
    ...
    //var_dump($_SESSION['sms_phone_code']);die;
    $msg = $code;
    //echo $msg;你的验证码是683923
    $url = "http://dx.10659com.com:83/ApiService.asmx/Send";
    //account=test&pwd=test&product=1060000123&mobile=13800000000,1370000000&message=测试
    $data = array(
        'account' => 'xiudangwang',
        'pwd' => '789789',
        'product' => '15',
        'mobile' => $phone,
        'message' => iconv('UTF-8','GB2312','您好！您的验证码是'.$code.'。【国安商城】')
    );
    ...
    if($arr[2] == 200){//000是接口的返回值
        ajaxReturn(array('ret_code' => 1, 'msg' => '发送成功', 'data' => array()),'JSON');
    }else{
        ajaxReturn(array('ret_code' => 0, 'msg' => '发送失败', 'data' => array()),'JSON');
    }
...
if($action == 'yanzhneg'){
        //echo 123;
    $codes = $_SESSION['sms_phone_code'];

        $yanzhengma = $_POST['yanzhengma'];
        if($codes == (int)$yanzhengma){
            $res = array('error'=>1,'msg'=>'验证码正确!');
        }else{
            $res = array('error'=>0,'msg'=>'验证码错误!');
        }
    ...
    if($action == 'tuihuan'){
        //echo 123;
        $order_id = $_POST['id'];
        $sql = "update " .$ecs->table('order_info')."  set is_tui = 1 where order_id = '$order_id' ";
...
if($action == 'zhenshi'){
        //echo 123;
        $name = $_POST['id'];
        $ID_number = $_POST['zhenshi'];
        if($user_id <= 0){
            $res = array('error'=>0,'msg'=>'此功能需要登录才能操作!');
            echo json_encode($res);die;
        }
        //给用户插入真实姓名和身份证号码
        $sql = "update ".$ecs->table('users')." set real_name='$ID_number',ID_number='$name' where user_id = ".$user_id;
        if($db->query($sql)){
            $res = array('error'=>1,'msg'=>'验证成功！');
            echo json_encode($res);die;
        }else{
            $res = array('error'=>2,'msg'=>'验证失败!');
            echo json_encode($res);die;
        }
    }
...
if ($action == 'default')
{
    ...
    //统计用户未付款订单
    $sql = "select count(*) from " .$ecs->table('order_info')." where user_id = '$user_id' and pay_status != 2";
    $pay_status = $db->getOne($sql);
    //统计用户未评价订单
    $sql = "select count(*) from " .$ecs->table('order_info')." where user_id = '$user_id' and pay_status = 2 and order_status = 5 and shipping_status = 2 and is_comment = 0";
    $is_comment = $db->getOne($sql);
    //统计用户已完成订单
    $sql = "select count(*) from " .$ecs->table('order_info')." where user_id = '$user_id' and pay_status = 2 and order_status = 5 and shipping_status = 2 and is_comment = 1";


    $goods_best = get_recommend_goods('best');
    foreach ($goods_best as $key => $value) {
        $sql = " select count(*) from ".$ecs->table('order_goods')." as a LEFT JOIN ".$ecs->table('order_info')." as b on a.order_id = b.order_id where b.shipping_status = 2 and a.goods_id = ".$value['id'];
        $dd = $db->getOne($sql);
        $goods_best[$key]['yishou'] = $dd[0];
        //统计所有评价
        $sql = "select count(*) from ".$ecs->table('comment')." where id_value = ".$value['id'];
        $pin_z = $db->getOne($sql);
        //var_dump($pin_z);
        //统计所有的好好评
        $sql = "select count(*) from ".$ecs->table('comment')." where comment_rank = 5 and id_value = ".$value['id'];
        $pin_h = $db->getOne($sql);
        if($pin_z[0] == 0){
           $goods_best[$key]['haopin'] = 100; 
        }else{
            $goods_best[$key]['haopin'] = ($pin_h[0]/$pin_z[0])*100;
        }
        //var_dump($pin_h);
    ...
    //var_dump($goods_best);die;
     $smarty->assign('best_goods',$goods_best); 
    $order = $db->getOne($sql);
    $smarty->assign('is_comment',$is_comment);
    $smarty->assign('order',$order);
    $smarty->assign('name',$_SESSION['user_name']);
...
if ($action == 'jifen')
{
	...
	$user_id = $_SESSION['user_id'];
    $jifen = $db->getOne("SELECT pay_points FROM " .$ecs->table('users'). " WHERE user_id = '$user_id'");
    $jifens = $jifen/10;
    $smarty->assign('jifens',  $jifens);
    $smarty->assign('jifen',  $jifen);
	//$where = " WHERE user_id = '$user_id' ";
	/* 查询记录 */
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('account_log'). " WHERE user_id = '$user_id'");

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);
    $account = get_user_account($user_id, $pager['size'], $pager['start']);
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $phones = isset($_POST['phonessss']) ? trim($_POST['phonessss']) : '';
    ...
    $_REQUEST['data'] = 'http://http://127.0.0.1/guoa/?='.$_SESSION['user_id'];
...
/* 验证用户手机是否被注册 */
elseif($action == 'check_phones')
{
    $phones = trim($_GET['phones']);

    $sql = " select mobile_phone from " .$ecs->table('users') ."where mobile_phone = '$phones'";
...
elseif ($action == 'qiandao')
{
    ...
    if(empty($_SESSION['user_id'])){
        ...
        $jifen_s['error'] = '0';
        echo json_encode($jifen_s);
    }else{
        //判断用户是否是连续登录签到
        $sql = " select * from ".$ecs->table('account_log')." where change_time < ".strtotime(date("Y-m-d"))." and change_time > ".(strtotime(date("Y-m-d"))-86400)." and change_type = 0 and user_id = ".$id;
        ...
        if($shijian){
                ...
                if(!$qiandao){
                    //var_dump($shijian['change_time']);die;
                    ...
                    $shi = time();
                    $sql =" insert into ".$ecs->table('account_log')."(user_id,pay_points,change_type,change_desc,change_time) values('$_SESSION[user_id]','$jifen',0,'登录签到获得积分','$shi')";
                    $db->query($sql);
                    //记录签到总积分
                    $sql = " select * from ".$ecs->table('sign')." where user_id = ".$id;
                    $user_id = $db->getRow($sql);
                    //修改总积分
                    $sql = " update ".$ecs->table('sign')." set sign_z = ".($user_id['sign_z']+$jifen)." where user_id = ".$id;
                    $db->query($sql);
                    $jifen_s['zong'] = $user_id['sign_z']+$jifen;
                    $jifen_s['jin'] = $jifen;
                    $jifen_s['error'] = '1';
                ...
                $qiandao = $db->getRow($sql);
                if(!$qiandao){
                        //var_dump($qiandao);die;
                        $shi = time();

                        $sql =" insert into ".$ecs->table('account_log')."(user_id,pay_points,change_type,change_desc,change_time) values('$_SESSION[user_id]','$_CFG[jifen_s]',0,'登录签到获得积分','$shi')";
                        $db->query($sql);
                        //记录签到总积分
                        ...
                        $sql = " update ".$ecs->table('sign')." set sign_z = ".($user_id['sign_z']+$_CFG['jifen_s'])." where user_id = ".$id;
                    }else{
                        $sql = "insert into  ".$ecs->table('sign')."(user_id,sign_z) values($id,'$_CFG[jifen_s]')";
                    }
                        $db->query($sql);
                        $jifen_s['zong'] = $user_id['sign_z']+$_CFG['jifen_s'];
...
elseif ($action == 'profile')
{
    ...
    //查询用户信息
    $sql = "select * from " .$ecs->table('users'). " where user_name = '$_SESSION[user_name]'";
...
elseif ($action == 'act_edit_profile')
{
    ...
    /* 处理图片 */
    $dd = date("Y-m/d");
    $litpic = basename($image->upload_image($_FILES['litpic'],'litpic/'.$dd));
    $images = 'litpic/'.$dd.'/'.$litpic;
    if (!empty($images))
    {
        //有图片上传
        $param = " ,head_portrait = '$images' ";
    }
    ...
    //var_dump($_FILES);die;
    //直接修改用户资料
    $sql = "UPDATE " .$ecs->table('users'). " set birthday = '$birthday',email = '$email',mobile_phone = '$mobile_phone',sex = '$sex',nickname = '$nickname',real_name = '$real_name',ID_number = '$ID_number',youbian = '$postcode', chanzhu = '$address' $param where user_id = ".$_SESSION['user_id'];
    $id = $db->query($sql);
    if($id){
        show_message('修改成功','', '', 'info');
    }else{
        show_message('修改失败','', '', 'info');
    }
...
elseif ($action == 'order_list')
{
    ...
    if($_POST){
        ...
        if($textfield != '0'){
            $where .= " order_sn LIKE '%$textfield%' and ";
            $smarty->assign('textfield',  $textfield);
        }
        $shijian = time();
        if($time != '0'){
            if($time == '一周内'){
                $where .= "add_time > ($shijian-604800) and ";
            }elseif($time == '15天内'){
                $where .= "add_time > ($shijian-1296000) and ";
            }
            elseif($time == '30天内'){
                $where .= "add_time > ($shijian-2592000) and ";
            }
        ...
        if($status != '0'){
            ...
            if($status == '待收货'){
                ...
                $where .= "order_status = 5 and shipping_status = 1 and pay_status = 2  and ";
            }elseif($status == '待评论'){
                ...
                $where .= "(order_status = 1 and shipping_status = 0 and pay_status = 2) || (order_status = 1 and shipping_status = 3 and pay_status = 2) || (order_status = 5 and shipping_status = 5 and pay_status = 2)  and ";
            }elseif($status == '待付款'){
                $where .= "(order_status = 0 and shipping_status = 0 and pay_status = 0) || (order_status = 1 and shipping_status = 0 and pay_status = 0)  and ";
            }
...
elseif ($action == 'address_list')
{
    ...
    foreach ($consignee_list as $key => $value) {
        ...
        $consignee_list[$key]['xiangxi2'] = huoqudiqu($value['country']);
        $consignee_list[$key]['xiangxi3'] = huoqudiqu($value['province']);
        $consignee_list[$key]['xiangxi4'] = huoqudiqu($value['city']);
        $consignee_list[$key]['xiangxi5'] = huoqudiqu($value['district']);
    ...
    $smarty->assign('count_address', count($consignee_list));
...
elseif ($action == 'comment_list')
{
    ...
    //var_dump(get_comment_list($user_id, $pager['size'], $pager['start']));die;
    $smarty->assign('pager',        $pager);
    $smarty->display('user_clips.dwt');
}
...
elseif ($action == 'act_add_message')
{
    ...
    {
        show_message($_LANG['add_message_success'], '返回', 'user.php?act=order_list&order_id=' . $message['order_id'],'info');
    }
    else
    {
        $err->show('返回', 'user.php?act=order_list');
    }
}
...
elseif ($action == 'booking_list')
{
    ...
    if($_POST){
        ...
        $textfield = $_POST['textfield'];
        if($textfield != '0'){
            $where .= " bg.order_sn LIKE '%$textfield%' and ";
            $smarty->assign('textfield',  $textfield);
        }
        $shijian = time();
        if($time != '0'){
            if($time == '一周内'){
                $where .= "booking_time > ($shijian-604800) and ";
            }elseif($time == '15天内'){
                $where .= "booking_time > ($shijian-1296000) and ";
            }
            elseif($time == '30天内'){
                $where .= "booking_time > ($shijian-2592000) and ";
            }
            $smarty->assign('time',  $time);
    ...
    $sql = "SELECT COUNT(*) " .
            ...
            "WHERE ".$where." bg.goods_id = g.goods_id AND user_id = '$user_id'";
    $record_count = $db->getOne($sql);
    $pager = get_pager('user.php', array('act' => $action), $record_count, $page);
    $smarty->assign('booking_list', get_booking_list($user_id, $pager['size'], $pager['start'],$where));
...
elseif ($action == 'affiliate')
{
    ...
    $sql =" select QR_code from ". $ecs->table('users')." where user_id = ".$user_id;
    $QR_code = $db->getOne($sql);
...
elseif ($action == 'fasong')
{
    ...
    include_once(ROOT_PATH .'includes/lib_passport.php');
    $json = new JSON();
    $email = trim($_POST['emailList']);
    $content = $_POST['content'];

    if (send_mail('', $email, '邀请邮件', $content, 0))
    {
        //make_json_result('', $_LANG['sendemail_success'] . $email);
        $res = array('error'=>0,'con'=>'已发送到'.$email);
    }
    else
    {
        $res = array('error'=>1,'con'=>'发送邮件到'.$email.'失败！！');
    }
