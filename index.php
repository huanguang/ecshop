...
//判断是否有随机显示查询要求
if($_POST['act']  == 'huanyip'){
    $idsss = $_POST['idd'];
    
    $as = array(0,1,2,3,4,5,6,7);
    ...
    switch ($ass)
        {
        case 0:
          $idsss = 'order by act_id desc';
          $res['error'] = '0';
          break;  
        case 1:
          $idsss = 'order by goods_id desc';
          $res['error'] = '1';
          break;
          case 2:
          $idsss = 'order by end_time desc';
          $res['error'] = '2';
          break;
          case 3:
          $idsss = 'order by start_time desc';
          $res['error'] = '3';
          case 4:
          $idsss = 'order by goods_id asc';
          $res['error'] = '4';
          case 5:
          $idsss = 'order by end_time asc';
          $res['error'] = '5';
          case 6:
          $idsss = 'order by start_time asc';
          $res['error'] = '6';
          case 7:
          $idsss = 'order by act_id asc';
          $res['error'] = '7';
          break;
        default:
          $idsss = 'order by act_id desc';
        ...
        $tuangou = index_get_group_buy2($idsss);
        $msg = '';
        foreach ($tuangou as $key => $value) {
                $msg .='<li>
                <div class="pro_time" yyear="2015" ymonth="7" yday="31" yhour="18" yminute="">
                    <p>
                        <i class="time_icon comm"></i>倒计时：<font class="leaveTime endtime" value="'.$value[end_time].'" showday="show"></font>
                    </p>
                    <span></span>
                    <s></s>
                 </div>
                        <div class="pic"><a href="'.$value[url].'">
                        <img src="'.$value[thumb].'" width="185" height="160" alt="'.$value[goods_name].'" /></a></div>
                        <div class="wz">
                            <div class="tit">
                            <a href="'.$value[url].'">'.$value[goods_name].'</a></div>
                          <div class="jg">
                                <span>￥'.$value['shop_price'].'</span>
                                <s>￥'.$value['market_price'].'</s>
                            </div>
                            <div class="sl">数量：'.$value[ext_info][restrict_amount].'</div>
                      </div>
                        <div class="ljqg">
                        <input type="button" value="立即抢购" onclick="window.location.href='.$value[url].'"></div>
                    </li>';
        }
        $res['msg'] = $msg;
...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    /**  获得6F婚宴专区       开始  */
    $sql = 'SELECT cat_id, goods_id, goods_name, market_price, shop_price, goods_img  FROM '.$ecs->table('goods').' WHERE cat_id = 158 and is_delete = 0 ORDER BY add_time desc';
    ...
    /**  获得7F礼盒馈赠       开始  */
    $sql = 'SELECT cat_id, goods_id, goods_name, market_price, shop_price, goods_img  FROM '.$ecs->table('goods').' WHERE cat_id = 159 and is_delete = 0 ORDER BY add_time desc';
    //SELECT * FROM `ecshop_miqi`.`ecs_goods` WHERE cat_id = 158 ORDER BY cat_id ASC
    ...
    /**  获得8F整箱分类，组合和整箱数据       开始  */
    $sql = 'SELECT goods_name, goods_id, shop_price, market_price, goods_img  FROM '.$ecs->table('goods') .' WHERE cat_id IN(153, 154) and is_delete = 0 order by add_time desc' ;
   ...
   // $smarty->assign('new_articles',    index_get_new_articles_bak());   // 最新文章
    ...
    //查询首页5f酒具商品
    $sql = 'SELECT add_time, cat_id, goods_img, goods_name, goods_desc, market_price, shop_price, promote_start_date, promote_end_date, goods_id  FROM '.$ecs->table('goods').' WHERE cat_id = 157 and is_delete = 0 ORDER BY add_time DESC LIMIT 0,1';
    ...
    //查询分类名称
    $sql ="select cat_name from " .$GLOBALS['ecs']->table('category'). " where cat_id = 157";
    $cat_5f = $GLOBALS['db']->getOne($sql);
     $smarty->assign('cat_5f', $cat_5f);

    $smarty->assign('pro_5f_jj_02', $pro_5f_jj_02);


    //查询首页5f酒具商品
    $sql = 'SELECT add_time, cat_id, goods_img, goods_name, goods_desc, market_price, shop_price, promote_start_date, promote_end_date, goods_id  FROM '.$ecs->table('goods').' WHERE cat_id = 157 and is_delete = 0 ORDER BY add_time DESC LIMIT 2,1';
    ...
    //查询促销商品列表 
    $sql = 'SELECT add_time, cat_id,goods_brief, goods_img, goods_name, goods_desc, market_price, shop_price, promote_start_date, promote_end_date, goods_id  FROM '.$ecs->table('goods').' WHERE cat_id = 156 and is_delete = 0 ORDER BY add_time DESC LIMIT 2';
    ...
    //5f精品酒具    组合商品列表
    $sql = 'SELECT add_time, cat_id, goods_img, goods_name, goods_desc, market_price, shop_price, promote_start_date, promote_end_date, goods_id  FROM '.$ecs->table('goods').' WHERE cat_id = 157 and is_delete = 0 ORDER BY add_time DESC LIMIT 3';
    ...
    //8f分类名
    $sql ="select cat_name FROM " .$GLOBALS['ecs']->table('category'). " where cat_id = 153 ";
    $cat_8fone = $GLOBALS['db']->getOne($sql);
    $sql ="select cat_name FROM " .$GLOBALS['ecs']->table('category'). " where cat_id = 154 ";
    $cat_8ftwo = $GLOBALS['db']->getOne($sql);
    $smarty->assign('cat_8fone',$cat_8fone);
    $smarty->assign('cat_8ftwo',$cat_8ftwo);
    aa();
    /* links */
 ...
 * @return  array
 */
function index_get_group_buy2($where)
{   //echo 123;die;
    $time = gmtime();
    $limit = get_library_number('group_buy', 'index');
