...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('article'). " WHERE cat_id = '$article_id'");
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
    $pager  = get_pager('culture_list.php', array('id' => $article_id), $record_count, $page);
    //var_dump($pager);die;


//$brand = isset($_REQUEST['brand']) && intval($_REQUEST['brand']) > 0 ? intval($_REQUEST['brand']) : 0;
    $smarty->assign('class_articles', index_get_class_articles_er($article_id,100,$pager['size'], $pager['start'])); // 分类调用文章 
    //var_dump(index_get_class_articles($article_id,100));die;
    ...
    $smarty->display('culture_list.dwt', $cache_id);
