...
if ($act == 'ajax')
{
    ...
    $sql = " insert into " .$ecs->table('comment'). " (comment_type,id_value,user_name,content,comment_rank,add_time,ip_address,user_id) values(0,'$goods_id','$name','$conmet','$pinfen','$times','$ip','$user_id')";
    $id = $db->query($sql);
    if($id){
        echo '1';die;
    }else{
...
if (!$smarty->is_cached('culture.dwt', $cache_id))
{
    assign_template();
    ...
    $smarty->assign('user_id',$user_id); // 会员id；20151128添加
	/**小图 end**/
    //查询专题评论的商品和所有评论
    $sql = " select goods_id,goods_name,goods_brief from " .$ecs->table('goods'). " where zhuanti = 1 limit 1";
    $goodsid = $db->getRow($sql);
    //查询评论
    $sql = " select a.*,b.user_id,b.user_name,b.head_portrait from " .$ecs->table('comment'). " as a LEFT JOIN " .$ecs->table('users'). " as b on a.user_id = b.user_id where id_value = ".$goodsid['goods_id']." and status = 1";
    $comment_list = $db->getAll($sql);
    //查出每一条评论的回复
    foreach ($comment_list as $key => $value) {
        $sql = " select * from " .$ecs->table('comment'). " where parent_id = " .$value['comment_id'];
        $comment_list[$key]['parent_id'] = $db->getAll($sql); 
        $comment_list[$key]['add_time']  = local_date('Y-m-d H:i:s',$value['add_time']);
    }
    //var_dump($comment_list);die;
    $smarty->assign('comment_list',$comment_list);       // 专题评论信息
    $smarty->assign('goods',$goodsid);       // 专题评论信息
...
/*
   ...
   // $smarty->assign('new_articles',    index_get_new_articles_bak());   // 最新文章
    ...
    //查询促销商品列表 
    $sql = 'SELECT add_time, cat_id, goods_img, goods_name, goods_desc, market_price, shop_price, promote_start_date, promote_end_date, goods_id  FROM '.$ecs->table('goods').' WHERE cat_id = 156 ORDER BY add_time DESC LIMIT 2';
    ...
    /* 首页主广告设置 20151123 */
        //大课堂广告轮播图
    $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 34 AND enabled = 1 ';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_d = $db->getAll($sql);
    //var_dump($ad_d);die;
    $smarty->assign('ad_d',$ad_d);

    /* 首页主广告设置 20151123 */
        //大课堂广告轮播图
    $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 35 AND enabled = 1 ';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_d2 = $db->getRow($sql);
    $smarty->assign('ad_d2',$ad_d2);

    /* 首页主广告设置 20151123 */
        //大课堂广告轮播图
    $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 36 AND enabled = 1 ';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_d3 = $db->getRow($sql);
    $smarty->assign('ad_d3',$ad_d3);
    /* 首页主广告设置 20151123 */
        //大课堂广告轮播图
    $sql = 'SELECT ad_link,ad_code FROM '. $ecs->table("ad").' WHERE position_id = 37 AND enabled = 1 ';
    //SELECT ad_link,ad_code FROM `ecshop_miqi`.`ecs_ad` WHERE position_id = 14 AND enabled = 1
    $ad_d4 = $db->getRow($sql);
    $smarty->assign('ad_d4',$ad_d4);

    //查询分类文章1美妆教师
    $smarty->assign('class_articles_32', index_get_class_articles(32,8));
    //查询分类文章2保健常识
    $smarty->assign('class_articles_33', index_get_class_articles(33,8));
    //查询分类文章3导师推荐
    $smarty->assign('class_articles_34', index_get_class_articles(34,8));
    ...
    $smarty->assign('helps_footer',           get_shop_help_footer());       // 网店帮助
$smarty->display('culture.dwt', $cache_id);
