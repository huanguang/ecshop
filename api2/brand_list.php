...
if (empty($brand_id))
{
    ...
    if (!$smarty->is_cached('brand_list.dwt', $cache_id))
    {
        ...
        //获取所有的品牌信息
        $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('brand') . " WHERE 1";

    $brand_list = $GLOBALS['db']->getAll($sql);
    $smarty->assign('brand_list',$brand_list);

    //获取所有的品牌下的促销商品
        $sqls = 'SELECT * FROM ' . $GLOBALS['ecs']->table('brand') . " WHERE 1";

    $brand_lists = $GLOBALS['db']->getAll($sqls);
    foreach ($brand_list as $key => $value) {
        $sql = " select * from "  . $GLOBALS['ecs']->table('goods') . " where promote_end_date > 0 and promote_start_date > 0 and brand_id = ".$value['brand_id'];
        //$brand_list = $GLOBALS['db']->getAll($sql);
        $brand_lists[$key]['goods'] = $GLOBALS['db']->getAll($sql);;
    }
    //print_r($brand_lists);die;
    $smarty->assign('brand_lists',$brand_lists);
    ...
    $smarty->display('brand_lists.dwt', $cache_id);
    exit();
}
