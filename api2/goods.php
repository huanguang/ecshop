...
//-- 商品收藏删除
/*------------------------------------------------------ */
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'delshou'){
    $user_id = $_SESSION['user_id'];
    $goods_id = $_POST['id'];
    $sql = "delete from ". $ecs->table('collect_goods') . " where user_id = '$user_id' and goods_id = '$goods_id'";
    $id = $db->query($sql);
    if($id){
        echo '1';die;
    }else{
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    //统计商品总的评论信息
    $sql = "select count(*) from ".$ecs->table('comment'). " where status = 1 and  id_value = ".$goods_id;
    $id = $db->getOne($sql);
    $smarty->assign('id', $id);
    ...
    else
    {
        ...
        $smarty->assign('properties',          $properties['pro']['商品属性']);                              // 商品属性
        //print_r($properties['pro']['商品属性']);die;
        ...
        $user_id = $_SESSION['user_id'];
                // 关联文章
        $shou = $db->getOne("SELECT rec_id FROM " .$ecs->table('collect_goods').
                                " WHERE user_id='$user_id' and goods_id = '$goods_id'");
    //var_dump($user_id[0]['ass'][0]['goods_list']);die;
    $smarty->assign('shou', $shou);


        //查询商品属性
        $sql = 'SELECT goods_id, goods_name, goods_desc  FROM '.$ecs->table('goods').' WHERE goods_id = '.$goods['goods_id'].' ORDER BY add_time DESC';
        $pro_sx = $db->getRow($sql);
        //print_r($pro_sx);die;


            //添加产地 口感属性 
            $sql = 'SELECT g.goods_id, a.goods_id, a.attr_value FROM '.$ecs->table('goods').'AS g LEFT JOIN '.$ecs->table('goods_attr').' AS a ON g.goods_id = a.goods_id WHERE a.goods_id = '.$pro_sx['goods_id']; //echo $sql;die;
           //print_R($pro_sx['goods_id']);die;
        ...
        $smarty->assign('pro_sx', $pro_sx);

        $smarty->assign('fittings',            get_goods_fittings(array($goods_id)));                   // 配件
        //print_r(get_goods_fittings(array($goods_id)));die;
