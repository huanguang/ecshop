...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ad',$ad);
        ...
        //查询最新中奖记录
        $sql = " select * from ".$ecs->table("lottery_user")." as a left JOIN ".$ecs->table("lottery"). " as b on a.lid = b.id ORDER BY a.id DESC";
...
include_once('includes/lib_clips.php');
    ...
    if(checkuserpoint($user_id,10) == "-1")
        {
    ...
    //查询中奖信息
        ...
        if($res['lg'] == '3'){
            //给用户添加优惠券
            $sql =" insert into ".$GLOBALS['ecs']->table('user_bonus'). " (bonus_type_id,user_id,emailed) values(15,$user_id,1)";
            $GLOBALS['db']->query($sql);
        }
        //四等奖    用户抽奖中的是实物奖品（优惠券10）
        if($res['lg'] == '4'){
            //给用户添加优惠券
            $sql =" insert into ".$GLOBALS['ecs']->table('user_bonus'). " (bonus_type_id,user_id,emailed) values(14,$user_id,1)";
            $GLOBALS['db']->query($sql);
        }
...
function checkuserpoint($user_id, $point)
{
    ...
    {   
        ...
        //并增加消费记录
        $shijian = time();
        $sql = "insert into ".$GLOBALS['ecs']->table('account_log')." (user_id,pay_points,change_time,change_desc,change_type) values($user_id,-10,$shijian,'积分抽奖',2)";
        $GLOBALS['db']->query($sql);
        return "1";
    }
    return "-1";
