...
/*------------------------------------------------------ */
    ...
    if($_REQUEST['act'] == 'ajax'){
        ...
        //查询用户是否点赞
        $sql = " select * from ".$ecs->table('praise'). " where user_id = '$user_id' and id = '$id'";
        $ids = $db->getRow($sql);

        if($ids){
            exit('3');
        }else{
            $sql = "insert into ". $ecs->table('praise') ."(id,user_id)  values('$id','$user_id')";
            if($db->query($sql)){
                $sql = "update ".$ecs->table('solicitation'). " set praise = '$dianzs' where id = '$id'";
                $db->query($sql);
                exit('1');
            }else{
                exit('0');
