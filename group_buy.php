...
if ($_REQUEST['act'] == 'list')
{
    ...
    if (!$smarty->is_cached('group_buy_list.dwt', $cache_id))
    {
        ...
        {
            /* 取得当前页的团购活动 */
            $gb_list = group_buy_list_bak($size, $page);
        ...
        //var_dump(get_categories_tree_pro());die;
...
//    }
    ...
    {   //echo 123;die;
        ...
        //统计团购信息
        $smarty->assign('comment',commentcount($goods_id));
        $smarty->assign('cfg', $_CFG);
        assign_template();
        ...
        //查询商品所属的品牌信息
        $sql ="select b.brand_desc from " .$ecs->table('goods'). " as a LEFT JOIN " .$ecs->table('brand'). " as b on a.brand_id = b.brand_id where a.goods_id = '$goods_id'";
        $brand_desc = $db->getOne($sql);
        $smarty->assign('brand_desc', $brand_desc);
        //统计商品总的评论信息
...
elseif ($_REQUEST['act'] == 'chaxun')
{
    $id=$_POST['id'];
    $sql = " select * from " .$ecs->table('goods_activity'). " where act_id = ".$id;
    $goods_list = $db->getRow($sql);
    $ass = unserialize($goods_list['ext_info']);
    echo $ass['restrict_amount'];die;
 ...
 * @return  array
 */
function group_buy_list_bak($size, $page)
{  
    /* 取得团购活动 */
    //$gb_list = array();
    $now = gmtime();
    $sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, b.act_id AS group_buy_id, g.market_price, g.cat_id ,".
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
    {
        ...
        /* 加入数组 */
        //print_R($group_buy);die;

        //$gb_list = array(cat_id => array());
        $gb_list[] = $group_buy;
    }
