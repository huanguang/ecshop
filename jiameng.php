...
$add_time = time();
//插入加盟数据表
$sql = 'insert into ' . $ecs->table('join_us'). " (name,iphone,email,dizhi,beizhu,add_time) values('$name','$iphone','$email','$dizhi','$beizhu','$add_time')";
if($db->query($sql)){
    show_message('提交成功，我们会尽快与您联系！');
}else{
    show_message('提交失败，请稍后再试！');
}
